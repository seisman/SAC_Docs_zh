...
Python 中使用 ``subprocess`` 模块的 ``Popen`` 方法调用 SAC，通过
``p.communicate()`` 将命令 ``s.encode()`` 传递给 SAC。
...
-  取三分量中的最大 ``b`` 和最小 ``e`` 值作为数据窗口，此操作要求三分量的
   ``kzdate`` 和 ``kztime`` 必须相同，这一点在添加事件信息时
...
下面的Python脚本中使用 :doc:`/commands/interpolate` 命令将所有数据重采样到相同
