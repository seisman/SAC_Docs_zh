...
\item 所有头段变量的变量名均以变量类型码开头，比如 \texttt{nvhdr} 是N型变量，
    \texttt{leven} 是L型变量；但F型变量除外；
\item 枚举型变量本质上是 \texttt{int} 型，只能在固定的几个值取值；
\item 逻辑型变量可以取值 \texttt{TRUE} 和 \texttt{FALSE}，本质上分别是1和0；
\item 字符型变量长度为8\footnote{C语言中用``\verb|\0|''作为字符串的结束标识符，因而源码中变量的实际长度还要加1}，
    只有 \texttt{kevnm} 很特殊，其长度为16；
